	...
	namespace JobSystem
	{
		...
		DWORD WINAPI JobRunner(void* i_pThreadInput)
		{
			...
			do
			{
				...
				if (pJob)
				{
					DEBUG_PRINT("JobRunner \"%s\": Starting Job \"%s\" on Processor %d.", pInput->m_ThreadName.c_str(), pJob->Name.c_str(), GetCurrentProcessorNumber());
					pInput->m_pQueue->StartingJob(pJob);
					pJob->Function();

					DEBUG_PRINT("JobRunner \"%s\": Finished Job \"%s\".", pInput->m_ThreadName.c_str(), pJob->Name.c_str());
					pInput->m_pQueue->FinishedJob();
					delete pJob;
				}
