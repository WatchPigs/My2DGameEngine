...
namespace GLib
{
	...
	void Sprite::Render(const Point2D& i_Offset, float i_Depth, float i_ZRotRadians) const
	{
		...
		DirectX::XMMATRIX mView = DirectX::XMMatrixTranslation(i_Offset.x, i_Offset.y, i_Depth);
	...
	bool Render(const Sprite& i_Sprite, const Point2D& i_Offset, float i_Depth, float i_ZRotRadians)
	{
		...
		i_Sprite.Render(i_Offset, i_Depth, i_ZRotRadians);
		return true;
	}
